...
export const CompactTemplate = ({ data, t, fontFamily, themeColor, fontSize }: { data: ResumeData, t: (key: string) => string; fontFamily: string; themeColor: string; fontSize: number; }) => {
  ...
  return (
    <div style={{ fontFamily, fontSize: `${fontSize}%` }} className="bg-white text-gray-800 p-8 w-full aspect-[210/297] text-[9.5pt] leading-snug shadow-2xl rounded-lg">
      {/* Header — dense single row */}
      <header className="mb-5 pb-4 border-b-2 border-gray-900">
      ...
      <main className="space-y-4">
        {summary && (
          <section>
            <h3 className="text-[10pt] font-bold uppercase tracking-wider text-gray-900 border-b border-gray-200 pb-1 mb-2">{t('summary')}</h3>
            <p className="text-[9.5pt] text-gray-700">{summary}</p>
          </section>
        )}

        <section>
          <h3 className="text-[10pt] font-bold uppercase tracking-wider text-gray-900 border-b border-gray-200 pb-1 mb-2">{t('experience')}</h3>
          <div className="divide-y divide-gray-200">
            {experience.map(exp => (
              <div key={exp.id} className="py-2 first:pt-0 last:pb-0">
                <div className="flex justify-between items-baseline gap-x-3">
                  <h4 className="font-bold text-[9.5pt] text-gray-900">{exp.title} <span className="font-medium text-gray-600">— {exp.company}{exp.city ? `, ${exp.city}` : ''}</span></h4>
        ...
        <section>
          <h3 className="text-[10pt] font-bold uppercase tracking-wider text-gray-900 border-b border-gray-200 pb-1 mb-2">{t('education')}</h3>
          <div className="divide-y divide-gray-200">
            {education.map(edu => (
              <div key={edu.id} className="py-1.5 first:pt-0 last:pb-0 flex justify-between items-baseline gap-x-3">
                <h4 className="font-bold text-[9.5pt] text-gray-900">{edu.degree} <span className="font-medium text-gray-600">— {edu.institution}{edu.city ? `, ${edu.city}` : ''}</span></h4>
        ...
        <section>
          <h3 className="text-[10pt] font-bold uppercase tracking-wider text-gray-900 border-b border-gray-200 pb-1 mb-2">{t('skills')}</h3>
          <ul className="flex flex-wrap gap-x-1.5 gap-y-0.5 text-[9pt] text-gray-700">
        ...
        <section>
          <h3 className="text-[10pt] font-bold uppercase tracking-wider text-gray-900 border-b border-gray-200 pb-1 mb-2">{t('languages')}</h3>
          <ul className="text-[9pt] text-gray-700">
